...
export interface LeafEnvelope {
  sessionId: string;
  workloadId?: string;        // SH workload that owns this run's sandbox pool
  sandboxPoolSelector?: string; // resolved by SH from workloadId before dispatch
...
/** Prefer the workload-specific pool resolved by SH; preserve the static deployment fallback. */
function poolEnv(env: LeafEnvelope): NodeJS.ProcessEnv {
...
export const realProduceSolve: ProduceSolve = async (env, config, capture) => {
  ...
  const selected = await selectPoolSandbox(poolEnv(env), cwd, sid, {
