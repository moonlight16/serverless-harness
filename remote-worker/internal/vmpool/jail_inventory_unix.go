...
// storage-layer proposal would be claiming to exploit. jailStats accumulates it into the
// owned and linked totals separately, so the gap can be read for the bytes this unlink
// actually frees rather than for a sum the shared snapshot components dominate.
