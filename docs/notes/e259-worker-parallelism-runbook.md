...
The **c=8** comparison is the headline: it is the single-worker peak and the known-healthy
operating point. c=16 and above are past the knee and already degraded, so a second worker there
measures recovery from saturation rather than clean scaling.
...
SH_MAX_COMMITTED_MB=409600        # two workers' standbys, with headroom; see below
...
admission budget, so two workers at 409600 can commit 800 GiB between them against 754 GiB of
RAM. Either drop each to ~262144, or confirm that combined residency stays inside physical
memory. A budget above installed memory means the gate cannot refuse before the OOM killer
arrives, and every density number becomes a measurement of the OOM killer.
